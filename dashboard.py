...
# Refresh otomatis setiap 1.5 detik
st_autorefresh(interval=1500, key="blynk_sync_loop")
...
for i, dev in enumerate(devices):
    with cols[i]:
        ...
        # Ambil nilai terbaru langsung dari server Blynk
        ...
        # Fungsi callback saat sakelar di klik oleh user di website
        def on_user_toggle(p=pin):
            current_state = st.session_state[f"toggle_{p}"]
            target_val = 1 if current_state else 0
            set_blynk_status(p, target_val)

        # Toggle sakelar dengan nilai yang selalu terikat ke status server
        st.toggle(
            dev["label"],
            value=is_active,
            key=f"toggle_{pin}",
            disabled=(server_val is None),
            on_change=on_user_toggle
        )
