...
# ==========================================
BLYNK_AUTH_TOKEN = "zW9oV9_a-PD7IXvfZ2b-8uhT8OgiEz8x"  # Auth Token Blynk
...
# ==========================================
# Refresh setiap 1.5 detik untuk sinkronisasi tombol fisik & Blynk
st_autorefresh(interval=1500, key="blynk_sync_counter")
...
for i, dev in enumerate(devices):
    with cols[i]:
        ...
        prev_key = f"prev_{pin}"

        # Ambil status langsung dari Blynk
        server_val = get_blynk_status(pin)
        server_state = (server_val == 1)

        # Inisialisasi status sebelumnya jika belum ada
        if prev_key not in st.session_state:
            st.session_state[prev_key] = server_state
            st.session_state[key_name] = server_state

        # Jika ada perubahan status dari tombol fisik / Blynk luar
        if server_val is not None and server_state != st.session_state[prev_key]:
            st.session_state[key_name] = server_state
            st.session_state[prev_key] = server_state

        # Toggle Switch
        current_toggle = st.toggle(
            dev["label"],
        ...
        # Deteksi aksi klik nyata dari pengguna di web dashboard
        if current_toggle != st.session_state[prev_key] and server_val is not None:
            new_val = 1 if current_toggle else 0
            if set_blynk_status(pin, new_val):
                st.session_state[key_name] = current_toggle
                st.session_state[prev_key] = current_toggle
                st.rerun()
